...
export async function generateMockData(count = 50, options = {}) {
  ...
  try {
    ...
    // Generate Reports
    if (shouldGenerate('reports') && personalInfoIds.length > 0) {
      logger.info("Beginning report generation");
      const reportsCount = await generateReports(count, userIds, personalInfoIds, demographicYears); // Pass userIds for submitter and personalInfoIds for reportedBy
      logger.info(`Generated ${reportsCount} report records`);
      stats.reports = reportsCount;
    } else {
      logger.info(`Skipping report generation ${personalInfoIds.length === 0 ? ' (no personal info IDs available)' : 'based on options'}.`);
    }
...
async function generateReports(count, userIds, personalInfoIds, demographicYears = []) {
  ...
  try {
    ...
    for (const reportData of reportsData.records) {
      try {
        ...
        // Assign a random person from available personal info IDs for reportedBy
        const reportedByPersonId = personalInfoIds[Math.floor(Math.random() * personalInfoIds.length)];
        reportData.reportedBy = reportedByPersonId;
