...
export const ACCOUNT_TYPES = {
  ...
  OPERATIONAL_EXPENDITURE: 'operational',
  GRANT: 'grant',
  REVENUE: 'revenue',
  TRANSFER: 'transfer'
};
...
// Validate a financial record for government compliance
export function validateGovernmentRecord(record) {
  const errors = [];
  const warnings = [];
  ...
  if (!record.approvalState) {
    warnings.push('No approval state specified, defaulting to draft');
  }
  
  // Check for budget reference
  if (!record.budgetReference) {
    warnings.push('No budget reference specified');
  }
  
  // Check for fiscal year
  if (!record.fiscalYear) {
    warnings.push('Fiscal year not specified');
  }
  
  // Check for spending justification
  if (!record.justification && record.total > 10000) {
    errors.push('Spending justification required for amounts over $10,000');
  }
  
  // Check for authorization reference for large amounts
  if (!record.authorizationReference && record.total > 50000) {
    errors.push('Legislative authorization reference required for amounts over $50,000');
  }
...
export function formatGovCurrency(amount, currency = 'USD', locale = 'en-US') {
